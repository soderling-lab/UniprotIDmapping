...
def submit_id_mapping(from_db, to_db, ids):
    request = requests.post(
        ...
        data={'taxonId': 10090, "from": from_db, "to": to_db, "ids": ",".join(ids),
              }
    )
    ...
    while True:
        ...
        if "jobStatus" in j:
            if j["jobStatus"] == "RUNNING":
                print(f"Retrying in {POLLING_INTERVAL}s", file=sys.stderr)
...
def main(GENE_INPUT, inputdb, outputdb, taxonID=None):
    global API_URL
    global session
    ...
    from_db=inputdb, to_db=outputdb, ids= GENE_INPUT
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ids', type=str, help='Please input a list of inputdb proteins', required=True)
    ...
    out = main(GENE_INPUT, args.inputdb, args.outputdb, args.taxonID)
    print(out)